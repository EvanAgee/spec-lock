...
- This repository is public. Never commit private repository names, host paths, other projects' spec or proof bodies, or fleet state. Test fixtures are synthetic repositories built in a temp directory.
...
- The hook's friendly name `spec-lock-reference-transaction` is a contract: the captain's one-shot escape is `git -c hook.spec-lock-reference-transaction.enabled=false <command>`. Renaming it breaks that escape.
