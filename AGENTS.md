...
- Tests: `npm test` (Node 22 `node:test`, no dependencies). Every test and manual walk uses a scratch `GIT_CONFIG_GLOBAL` with `GIT_CONFIG_NOSYSTEM=1`, strips inherited `GIT_*`, `FM_*` and `SPEC_LOCK_*` variables, and points `HOME` (which places the central file, `$HOME/.config/spec-lock/config`) and `SPEC_LOCK_LOG` into the temp directory; never register the hook in a real global config or repository, and never write the real central file, landing log or activation record. `bin/spec-lock-rollout activate` writes the global git config of whatever HOME and `GIT_CONFIG_GLOBAL` it runs with, so run it only under scratch ones; the real switch-on is the owner's.
...
- `action.yml` runs `bin/spec-lock-action` on GitHub, and consumers pin it by full commit SHA. The checker line (`spec-lock checker <version>+<hash>`) hashes `bin/spec-lock` and `lib/*.mjs`, so it changes whenever they do. `test/github-walk.mjs` (also `SPEC_LOCK_CANARY=<owner/repo> npm test`) pushes to a real repository and moves its main: point it only at the throwaway canary `EvanAgee/spec-lock-canary`, which is enrolled with `--integration integration/candidate` (its pre-T6 main is kept at `archive/t5-main`). `spec-lock-rollout enroll --apply` writes a real repository's workflow and rulesets: the canary is the only repository an agent may apply it to.
- The evidence binding in `README.md` ("Evidence bindings") is version 1 of a contract shared with an external run recorder that writes the manifests. Change its fields or rules only together with that recorder and under a new `version`; `test/evidence.test.mjs` holds its fixtures.
