...
// Adds the shared hooks to a git config file, one friendly name per event, so a single command can
// switch either off: git -c hook.spec-lock-reference-transaction.enabled=false <command>
export function register(configFile, binDir = BIN) {
  ...
  for (const [name, event] of [[HOOK, 'reference-transaction'], [PUSH_HOOK, 'pre-push']]) {
    execFileSync('git', ['config', '--file', configFile, `hook.${name}.command`, `${quoted} ${event}`])
    execFileSync('git', ['config', '--file', configFile, `hook.${name}.event`, event])
  }
}
